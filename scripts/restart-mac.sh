#!/usr/bin/env bash
# Reset FRIDAY like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${FRIDAY_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="FRIDAY.app/Contents/MacOS/FRIDAY"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/FRIDAY"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/FRIDAY"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/FRIDAY"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.molt.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/friday-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${FRIDAY_RESTART_LOG:-/tmp/friday-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${FRIDAY_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.friday/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  FRIDAY_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node friday.mjs daemon install --force --runtime node"
      log "  node friday.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.friday/disable-launchagent"
      log ""
...
kill_all_friday() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "FRIDAY" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
log "==> Killing existing FRIDAY instances"
kill_all_friday
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product FRIDAY"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/FRIDAY.app" ]]; then
    APP_BUNDLE="/Applications/FRIDAY.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/FRIDAY.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/FRIDAY.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/FRIDAY.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set FRIDAY_APP_BUNDLE to your installed FRIDAY.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node friday.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node friday.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".friday", "friday.json"), "utf8");
