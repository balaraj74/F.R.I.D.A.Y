...
# Build and bundle FRIDAY into a minimal .app we can open.
# Outputs to dist/FRIDAY.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/FRIDAY.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/friday/friday/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/FRIDAY/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :FRIDAYBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :FRIDAYGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/FRIDAY"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/FRIDAY"
fi
chmod +x "$APP_ROOT/Contents/MacOS/FRIDAY"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/FRIDAY" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/FRIDAY/Resources/FRIDAY.icns" "$APP_ROOT/Contents/Resources/FRIDAY.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/FRIDAY/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying FRIDAYKit resources"
FRIDAYKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/FRIDAYKit_FRIDAYKit.bundle"
if [ -d "$FRIDAYKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/FRIDAYKit_FRIDAYKit.bundle"
  cp -R "$FRIDAYKIT_BUNDLE" "$APP_ROOT/Contents/Resources/FRIDAYKit_FRIDAYKit.bundle"
else
  echo "WARN: FRIDAYKit resource bundle not found at $FRIDAYKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running FRIDAY"
killall -q FRIDAY 2>/dev/null || true
