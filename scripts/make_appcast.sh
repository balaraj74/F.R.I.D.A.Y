...
ZIP=${1:?"Usage: $0 FRIDAY-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/friday/friday/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^FRIDAY-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/friday/friday/releases/download/v${VERSION}/"}
