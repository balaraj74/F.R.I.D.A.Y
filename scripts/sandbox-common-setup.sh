...
BASE_IMAGE="${BASE_IMAGE:-friday-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-friday-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=friday.sandbox=1)
NOTE
