...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/friday/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "FRIDAY Re-Auth" -c "After getting key, run: ssh $SERVER '~/friday/scripts/mobile-reauth.sh'" --id friday-auth
        ;;
    *)
