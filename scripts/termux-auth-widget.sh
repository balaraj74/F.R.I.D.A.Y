...
SERVER="${FRIDAY_SERVER:-l36}"

# Check auth status
termux-toast "Checking FRIDAY auth..."

STATUS=$(ssh "$SERVER" '$HOME/friday/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/friday/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|FRIDAY_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/friday/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: FRIDAY won't work until re-auth"
                ;;
        esac
        ;;
