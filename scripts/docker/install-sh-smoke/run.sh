...
SMOKE_PREVIOUS_VERSION="${FRIDAY_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FRIDAY_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="friday"
if [[ -z "${FRIDAY_INSTALL_PACKAGE:-}" && "$INSTALL_URL" == *"clawd.bot"* ]]; then
  DEFAULT_PACKAGE="friday"
fi
PACKAGE_NAME="${FRIDAY_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "friday" ]]; then
  ALT_PACKAGE_NAME="friday"
else
  ALT_PACKAGE_NAME="friday"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FRIDAY_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FRIDAY_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$FRIDAY_INSTALL_LATEST_OUT"
fi
