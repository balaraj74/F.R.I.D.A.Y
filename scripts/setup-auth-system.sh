#!/bin/bash
# Setup FRIDAY Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'friday-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  FRIDAY message: Send warning via FRIDAY itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/friday-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/friday-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/friday-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now friday-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status friday-auth-monitor.timer"
echo ""
echo "Quick commands:"
