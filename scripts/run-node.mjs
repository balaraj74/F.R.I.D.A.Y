...
const compiler = env.FRIDAY_TS_COMPILER === "tsc" ? "tsc" : "tsgo";
...
const shouldBuild = () => {
  if (env.FRIDAY_FORCE_BUILD === "1") return true;
...
const logRunner = (message) => {
  if (env.FRIDAY_RUNNER_LOG === "0") return;
  process.stderr.write(`[friday] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["friday.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
