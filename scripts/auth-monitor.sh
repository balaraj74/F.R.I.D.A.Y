...
#
# Suggested cron: */30 * * * * /home/admin/friday/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FRIDAY notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., friday-alerts)
...
STATE_FILE="$HOME/.friday/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FRIDAY if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use friday
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FRIDAY to $NOTIFY_PHONE..."
            friday send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FRIDAY is down. Run: ssh l36 '~/friday/scripts/mobile-reauth.sh'" "urgent"
    exit 1
