...
IMAGE_NAME="friday-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="friday-net-e2e-$$"
GW_NAME="friday-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "FRIDAY_GATEWAY_TOKEN=$TOKEN" \
	  -e "FRIDAY_SKIP_CHANNELS=1" \
	  -e "FRIDAY_SKIP_GMAIL_WATCHER=1" \
	  -e "FRIDAY_SKIP_CRON=1" \
	  -e "FRIDAY_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
