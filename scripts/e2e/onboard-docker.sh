...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/friday-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/friday-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/friday-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/friday-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.friday/friday.json"
    sessions_dir="$HOME/.friday/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.friday/friday.json" <<'"'"'JSON'"'"'
{
