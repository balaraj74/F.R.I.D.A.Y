...
IMAGE_NAME="friday-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/friday-bin:$PATH"
  mkdir -p /tmp/friday-bin

  cat > /tmp/friday-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/friday-bin/systemctl

  cat > /tmp/friday-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/friday-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/friday"
  npm_entry="/tmp/npm-prefix/lib/node_modules/friday/friday.mjs"
  git_entry="/app/friday.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/friday-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/friday-gateway.service"
    if [ ! -f "$unit_path" ]; then
