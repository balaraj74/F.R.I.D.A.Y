...
async function noteZaloTokenHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Docs: https://docs.friday.ai/channels/zalo",
    ].join("\n"),
    "Zalo bot token",
  );
}
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => setZaloDmPolicy(cfg as FRIDAYConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZaloAccountId(cfg as FRIDAYConfig);
...
export const zaloOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const configured = listZaloAccountIds(cfg as FRIDAYConfig).some((accountId) =>
      Boolean(resolveZaloAccount({ cfg: cfg as FRIDAYConfig, accountId }).token),
    );
    return {
      channel,
      configured,
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultZaloAccountId = resolveDefaultZaloAccountId(cfg as FRIDAYConfig);
