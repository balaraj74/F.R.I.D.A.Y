...
export function listGoogleChatAccountIds(cfg: FRIDAYConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: FRIDAYConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: FRIDAYConfig): ResolvedGoogleChatAccount[] {
