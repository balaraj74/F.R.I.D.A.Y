...
function resolveFRIDAYRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.FRIDAY_ROOT?.trim() || process.env.FRIDAY_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["friday", "friday"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set FRIDAY_ROOT (or legacy FRIDAY_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveFRIDAYRoot();
