...
FRIDAY extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
friday channels login --channel zalouser
...
```bash
friday message send --channel zalouser --target <threadId> --message "Hello from FRIDAY!"
```
...
```bash
friday channels login --channel zalouser              # Login via QR
friday channels login --channel zalouser --account work
friday channels status --probe
friday channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
friday directory self --channel zalouser
friday directory peers list --channel zalouser --query "name"
friday directory groups list --channel zalouser --query "work"
friday directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
friday message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
friday message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `friday channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
friday channels login --channel zalouser --account work
friday message send --channel zalouser --account work --target <id> --message "Hello"
