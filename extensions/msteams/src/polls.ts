...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["fridayPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["friday", "pollId"]) ??
    readNestedString(value, ["friday", "poll", "id"]) ??
    readNestedString(value, ["data", "fridayPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "friday", "pollId"]);
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "friday poll vote",
          displayText: "Vote recorded",
          value: { fridayPollId: pollId },
        },
      },
    ],
  };
