...
metadata: {"friday":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == FRIDAY `sessions_spawn`
- **File I/O** == FRIDAY `read`/`write`
- **Remote fetch** == FRIDAY `web_fetch` (or `exec` with curl when POST is required)
