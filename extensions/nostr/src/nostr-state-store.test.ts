...
import type { PluginRuntime } from "friday/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.FRIDAY_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-nostr-"));
  process.env.FRIDAY_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.FRIDAY_STATE_DIR?.trim();
  ...
  try {
    ...
    if (previous === undefined) delete process.env.FRIDAY_STATE_DIR;
    else process.env.FRIDAY_STATE_DIR = previous;
