/**
 * Twitch channel plugin for FRIDAY.
 *
...
import { buildChannelConfigSchema } from "friday/plugin-sdk";
 ...
 * for FRIDAY. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: FRIDAYConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: FRIDAYConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: FRIDAYConfig): boolean => {
