...
`friday.invoke --each`) without adding new FRIDAY code per workflow.
...
This extension depends on FRIDAY internal modules (the embedded agent runner).
It is intended to ship as a **bundled** FRIDAY extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.friday/extensions` as a standalone plugin directory.
