...
// NOTE: This extension is intended to be bundled with FRIDAY.
// When running from source (tests/dev), FRIDAY internals live under src/.
...
export function createLlmTaskTool(api: FRIDAYPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via friday.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-llm-task-"));
