...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.friday/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.friday/node.json` (node id + pairing token).
...
Path: `~/.friday/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.friday/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.friday/exec-approvals.sock` (0600).
