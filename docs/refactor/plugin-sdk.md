...
Delivery:
- Publish as `@friday/plugin-sdk` (or export from core under `friday/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `FRIDAYPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: FRIDAYConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: FRIDAYConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: FRIDAYConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: FRIDAYConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: FRIDAYConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: FRIDAYConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@friday/plugin-sdk`.
- Add `api.runtime` to `FRIDAYPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `fridayRuntime: ">=2026.2.0"`).
