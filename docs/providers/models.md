---
summary: "Model providers (LLMs) supported by FRIDAY"
read_when:
...
FRIDAY can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `friday onboard`).
