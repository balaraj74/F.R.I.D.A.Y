---
summary: "Use Synthetic's Anthropic-compatible API in FRIDAY"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. FRIDAY registers it as the
...
```bash
friday onboard --auth-choice synthetic-api-key
```
...
Note: FRIDAY's Anthropic client appends `/v1` to the base URL, so use
