---
summary: "Use Venice AI privacy-focused models in FRIDAY"
read_when:
  - You want privacy-focused inference in FRIDAY
...
```bash
friday onboard --auth-choice venice-api-key
```
...
```bash
friday chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, FRIDAY shows all available Venice models. Pick based on your needs:
...
```bash
friday models set venice/claude-opus-45
friday models set venice/llama-3.3-70b
```

List all available models:

```bash
friday models list | grep venice
```

## Configure via `friday configure`

1. Run `friday configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
FRIDAY automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
friday chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
friday chat --model venice/claude-opus-45

# Use uncensored model
friday chat --model venice/venice-uncensored

# Use vision model with image
friday chat --model venice/qwen3-vl-235b-a22b

# Use coding model
friday chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `friday models list` to see currently available models. Some models may be temporarily offline.
