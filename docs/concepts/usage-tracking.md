...
- `/usage cost` in chats: local cost summary aggregated from FRIDAY session logs.
- CLI: `friday status --usage` prints a full per-provider breakdown.
- CLI: `friday channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
