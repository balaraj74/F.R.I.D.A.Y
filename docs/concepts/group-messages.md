...
Add a `groupChat` block to `~/.friday/friday.json` so display-name pings work even when WhatsApp strips the visual `@` in the text body:
...
Notes:
- The regexes are case-insensitive; they cover a display-name ping like `@friday` and the raw number with or without `+`/spaces.
...
## How to use
1) Add your WhatsApp account (the one running FRIDAY) to the group.
2) Say `@friday …` (or include the number). Only allowlisted senders can trigger it unless you set `groupPolicy: "open"`.
...
- Manual smoke:
  - Send an `@friday` ping in the group and confirm a reply that references the sender name.
...
- Session store entries will appear as `agent:<agentId>:whatsapp:group:<jid>` in the session store (`~/.friday/agents/<agentId>/sessions/sessions.json` by default); a missing entry just means the group hasn’t triggered a run yet.
