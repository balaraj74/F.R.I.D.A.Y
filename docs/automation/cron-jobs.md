...
- Jobs persist under `~/.friday/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.friday/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.friday/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
friday cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
friday cron edit <jobId> --agent ops
friday cron edit <jobId> --clear-agent
```
```
...
```bash
friday system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`friday system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `FRIDAY_SKIP_CRON`.
