...
FRIDAY features that can generate provider usage or paid API calls.
...
- `friday status --usage` and `friday channels list` show provider **usage windows**
...
FRIDAY can pick up credentials from:
...
### 8) Model scan / probe
`friday models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
