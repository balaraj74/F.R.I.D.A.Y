...
This document explains how FRIDAY manages sessions end-to-end:
...
FRIDAY is designed around a single **Gateway process** that owns session state.
...
FRIDAY persists sessions in two layers:
...
- Transcripts: `~/.friday/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

FRIDAY resolves these via `src/config/sessions.ts`.
...
FRIDAY intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (FRIDAY consumes the events, but Pi decides when to compact).
...
FRIDAY also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, FRIDAY bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, FRIDAY leaves it alone.
...
FRIDAY supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- FRIDAY strips/suppresses this in the delivery layer.

As of `2026.1.10`, FRIDAY also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
FRIDAY uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but FRIDAY’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `friday status`.
