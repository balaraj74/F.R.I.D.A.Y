---
summary: "First-run onboarding flow for FRIDAY (macOS app)"
read_when:
...
- Writes credentials to `~/.friday/credentials/oauth.json`
...
The app can install the global `friday` CLI via npm/pnpm so terminal
...
On the first agent run, FRIDAY bootstraps a workspace (default `~/friday`):
...
```bash
friday webhooks gmail setup --account you@gmail.com
```
