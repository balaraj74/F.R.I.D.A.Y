...
read_when:
  - You are building a FRIDAY plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (friday.plugin.json)

Every plugin **must** ship a `friday.plugin.json` file in the **plugin root**.
FRIDAY uses this manifest to validate configuration **without executing plugin
