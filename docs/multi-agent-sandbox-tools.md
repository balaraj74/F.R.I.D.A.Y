...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `friday sandbox explain`.
...
Legacy `agent.*` configs are migrated by `friday doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=friday.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${FRIDAY_STATE_DIR:-$HOME/.friday}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
