...
- During heartbeat runs, FRIDAY treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `friday sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, FRIDAY skips the heartbeat run entirely (no model call).
...
```bash
friday system event --text "Check for urgent follow-ups" --mode now
```
