...
- Replaces the legacy `gateway` command. CLI entry point: `friday gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
friday gateway --port 18789
# for full debug/trace logs in stdio:
friday gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
friday gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.friday/friday.json` (or `FRIDAY_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__friday__/canvas/` from `~/friday/canvas`. Disable with `canvasHost.enabled=false` or `FRIDAY_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `FRIDAY_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `FRIDAY_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.friday.*` may still exist)
- Linux: `friday-gateway-<profile>.service`
- Windows: `FRIDAY Gateway (<profile>)`

Install metadata is embedded in the service config:
- `FRIDAY_SERVICE_MARKER=friday`
- `FRIDAY_SERVICE_KIND=gateway`
- `FRIDAY_SERVICE_VERSION=<version>`
...
```bash
friday --dev setup
friday --dev gateway --allow-unconfigured
...
- `FRIDAY_STATE_DIR=~/.friday-dev`
- `FRIDAY_CONFIG_PATH=~/.friday-dev/friday.json`
- `FRIDAY_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/friday-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):
- Base port = `gateway.port` (or `FRIDAY_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `FRIDAY_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `FRIDAY_CONFIG_PATH`
- unique `FRIDAY_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
friday --profile main gateway install
friday --profile rescue gateway install
```

Example:
```bash
FRIDAY_CONFIG_PATH=~/.friday/a.json FRIDAY_STATE_DIR=~/.friday-a friday gateway --port 19001
FRIDAY_CONFIG_PATH=~/.friday/b.json FRIDAY_STATE_DIR=~/.friday-b friday gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `friday health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `friday`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `friday gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.friday.*` is cleaned up).
  - `friday doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are FRIDAY profile services.
  ...
  - Cleanup: `friday gateway uninstall` (current service) and `friday doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `friday gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- FRIDAY.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.friday.*` labels still unload cleanly).
- To stop it cleanly, use `friday gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `friday gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `friday gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
FRIDAY installs a **systemd user service** by default on Linux/WSL2. We
...
`friday gateway install` writes the user unit. `friday doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/friday-gateway[-<profile>].service`:
```
[Unit]
Description=FRIDAY Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/friday gateway --port 18789
Restart=always
RestartSec=5
Environment=FRIDAY_GATEWAY_TOKEN=
...
```
systemctl --user enable --now friday-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/friday-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now friday-gateway[-<profile>].service
```
...
## CLI helpers
- `friday gateway health|status` — request health/status over the Gateway WS.
- `friday message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `friday agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `friday gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `friday gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `friday gateway` and the legacy TCP control port.
