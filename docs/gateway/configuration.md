---
summary: "All configuration options for ~/.friday/friday.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

FRIDAY reads an optional **JSON5** config from `~/.friday/friday.json` (comments + trailing commas allowed).

If the file is missing, FRIDAY uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/friday`). You usually only need a config to:
...
FRIDAY only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `friday doctor`, `friday logs`, `friday health`, `friday status`, `friday service`, `friday help`).
- Run `friday doctor` to see the exact issues.
- Run `friday doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `friday config set`. Keep a backup of `~/.friday/friday.json`.
...
```bash
friday gateway call config.get --params '{}' # capture payload.hash
friday gateway call config.apply --params '{
...
```bash
friday gateway call config.get --params '{}' # capture payload.hash
friday gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/friday" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@friday", "reisponde"] }
      }
    ]
  },
  channels: {
...
{ "$include": "/etc/friday/base.json5" } // absolute
...
```json5
// ~/.friday/clients/mueller/agents.json5
[
...
```json5
// ~/.friday/clients/mueller/broadcast.json5
{
...
FRIDAY reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, FRIDAY runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `FRIDAY_LOAD_SHELL_ENV=1`
- `FRIDAY_SHELL_ENV_TIMEOUT_MS=15000`
...
FRIDAY stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.friday/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.friday/credentials/oauth.json` (or `$FRIDAY_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.friday/agent/*` (migrated by `friday doctor` into `~/.friday/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `FRIDAY_OAUTH_DIR`
- Agent dir (default agent root override): `FRIDAY_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, FRIDAY imports `oauth.json` entries into `auth-profiles.json`.
...
If set, FRIDAY derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/friday/friday-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/friday/friday.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.friday/credentials/whatsapp/biz
          // authDir: "~/.friday/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `friday doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@friday", "friday", "friday"] } }
    ]
  }
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@friday"]
        }
      }
    ]
  }
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/friday-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.friday/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/friday-home" },
      { id: "work", workspace: "~/friday-work" }
    ]
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `friday.json`).
...
FRIDAY starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["friday-dm"]          // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {               // guild id (preferred) or slug
          slug: "friends-of-friday",
...
FRIDAY starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `friday plugins install @friday/mattermost` (or `./extensions/mattermost` from a git checkout).
...
FRIDAY starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
FRIDAY spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, FRIDAY
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/friday" } }
}
```
...
When a file exceeds this limit, FRIDAY logs a warning and injects a truncated
...
message envelopes). If unset, FRIDAY uses the host timezone at runtime.
...
`[friday]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[friday]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[friday]"`, FRIDAY will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, FRIDAY generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.friday/settings/tts.json",
      elevenlabs: {
...
FRIDAY also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, FRIDAY currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `friday configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:friday`: all built-in FRIDAY tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.friday/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.friday/sandboxes",
        docker: {
          ...
          apparmorProfile: "friday-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "friday-sandbox-browser:bookworm-slim",
          containerPrefix: "friday-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
FRIDAY uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.friday/agents/<agentId>/agent/models.json` or by defining the same schema inside your
FRIDAY config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, FRIDAY writes/merges a `models.json` into
`~/.friday/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. FRIDAY uses
...
- Shortcut: `friday onboard --auth-choice opencode-zen`.
...
Shortcut: `friday onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `friday onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `friday onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `friday onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `friday onboard --auth-choice minimax-api`.
...
- Override the agent config root with `FRIDAY_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.friday/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.friday/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.friday/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, FRIDAY stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.friday/skills` (workspace skills
...
from `~/.friday/extensions`, `<workspace>/.friday/extensions`, plus any
...
### `browser` (friday-managed browser)

FRIDAY can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for friday and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (FRIDAY.app menubar, or `friday gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      friday: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/friday" }
...
- Examples: `"/ui"`, `"/friday"`, `"/apps/friday"`.
...
- When a connection comes from one of these IPs, FRIDAY uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `friday gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `FRIDAY_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `FRIDAY_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. FRIDAY
...
macOS app behavior:
- FRIDAY.app watches `~/.friday/friday.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.friday/friday.json` (or `FRIDAY_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.friday/friday.json` (or `FRIDAY_CONFIG_PATH`)
...
- `FRIDAY_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `friday --dev …` → uses `~/.friday-dev` + shifts ports from base `19001`
- `friday --profile <name> …` → uses `~/.friday-<name>` (port via config/env/flags)
...
Example:
```bash
FRIDAY_CONFIG_PATH=~/.friday/a.json \
FRIDAY_STATE_DIR=~/.friday-a \
friday gateway --port 19001
```
...
Gmail helper config (used by `friday webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "friday@gmail.com",
...
- Set `FRIDAY_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, FRIDAY defaults `serve.path` to `/` so
...
Default root: `~/friday/canvas`  
Default port: `18793` (chosen to avoid the friday browser CDP port `18792`)  
The server listens on the **gateway bind host** (LAN or Tailnet) so nodes can reach it.

The server:
- serves files under `canvasHost.root`
- injects a tiny live-reload client into served HTML
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__friday/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__friday__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.friday/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.friday/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.friday/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_friday-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_friday-bridge._tcp` under `~/.friday/dns/` using the standard discovery domain `friday.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `friday.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `friday.internal` via that server
