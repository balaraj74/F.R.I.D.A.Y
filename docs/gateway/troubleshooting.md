---
summary: "Quick troubleshooting guide for common FRIDAY failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When FRIDAY misbehaves, here's how to fix it.
...
|---|---|---|
| `friday status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `friday status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `friday status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `friday gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `friday channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `friday gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `friday logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `friday status --all` (it redacts tokens). If you paste `friday status`, consider setting `FRIDAY_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  friday models auth setup-token --provider anthropic
  ```
...
```bash
friday models auth paste-token --provider anthropic
friday models status
```
...
**Logs:**
- Preferred: `friday logs --follow`
- File logs (always): `/tmp/friday/friday-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$FRIDAY_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u friday-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  friday config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  friday config set gateway.mode remote
  friday config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `friday setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.friday/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `friday doctor`
...
- then run `friday sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `friday gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `friday gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `friday gateway status` and `friday doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `FRIDAY_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `friday gateway status` shows a config mismatch**
...
- Fix: rerun `friday gateway install --force` from the same `--profile` / `FRIDAY_STATE_DIR` you want the service to use.

**If `friday gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `friday doctor` to update it (or `friday gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `FRIDAY_GATEWAY_TOKEN`) and restart the service.

**If `friday gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/friday` on disk.
...
Symptoms: `pwd` or file tools show `~/.friday/sandboxes/...` even though you
...
FRIDAY intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `friday models list` or
  `friday models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${FRIDAY_CONFIG_PATH:-$HOME/.friday/friday.json}"
```
...
tail -f "$(ls -t /tmp/friday/friday-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
friday logs --follow | grep "pairing request"
```
...
- ❌ `@friday` + image
- ✅ `@friday check this` + image

### Session Not Resuming

**Check 1:** Is the session file there?
```bash
ls -la ~/.friday/agents/<agentId>/sessions/
```
...
friday logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
friday gateway --verbose
```

If you’re logged out / unlinked:

```bash
friday channels logout
trash "${FRIDAY_STATE_DIR:-$HOME/.friday}/credentials" # if logout can't cleanly remove everything
friday channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/friday/friday-*.log | head -1)" | tail -20
```

### High Memory Usage

FRIDAY keeps conversation history in memory.
...
FRIDAY now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `friday doctor` reports every invalid entry.
- `friday doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `friday logs`, `friday health`, `friday status`, `friday gateway status`, and `friday gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/friday/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `friday models status` (CLI).
...
```bash
curl -fsSL https://friday.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `friday channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. FRIDAY scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/friday/friday/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
friday gateway status
friday gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.friday.* still works)
```
...
Ensure the global `friday` CLI is installed and matches the app version:
```bash
friday --version
npm install -g friday@<version>
```
...
#   ${FRIDAY_CONFIG_PATH:-$HOME/.friday/friday.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
friday gateway --verbose
friday channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/friday/friday-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$FRIDAY_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.friday/logs/...`; profiles use `~/.friday-<profile>/logs/...`)<br />Linux: `journalctl --user -u friday-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "FRIDAY Gateway (<profile>)" /V /FO LIST` |
| Session files | `$FRIDAY_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$FRIDAY_STATE_DIR/media/` |
| Credentials | `$FRIDAY_STATE_DIR/credentials/` |
...
trash "${FRIDAY_STATE_DIR:-$HOME/.friday}"
friday channels login         # re-pair WhatsApp
friday gateway restart           # or: friday gateway
```
...
1. Check logs first: `/tmp/friday/` (default: `friday-YYYY-MM-DD.log`, or your configured `logging.file`)
