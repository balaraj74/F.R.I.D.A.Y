...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when FRIDAY would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When FRIDAY would reply in this chat, it will run all three agents.
...
Workspace: /Users/pascal/friday-alfred/
...
Workspace: /Users/pascal/friday-baerbel/
Tools: read only
```
...
**Debug:**
```bash
tail -f ~/.friday/logs/gateway.log | grep broadcast
```
