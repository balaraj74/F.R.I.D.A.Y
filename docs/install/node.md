...
read_when:
  - "You installed FRIDAY but `friday` is “command not found”"
...
FRIDAY’s runtime baseline is **Node 22+**.

If you can run `npm install -g friday@latest` but later see `friday: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
