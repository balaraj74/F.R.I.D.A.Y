---
summary: "FRIDAY on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up FRIDAY on a Raspberry Pi
  - Running FRIDAY on ARM devices
  - Building a cheap always-on personal AI
---

# FRIDAY on Raspberry Pi

## Goal

Run a persistent, always-on FRIDAY Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most FRIDAY features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled friday
...
```bash
# Check logs
journalctl -u friday --no-pager -n 100

# Common fix: rebuild
cd ~/friday  # if using hackable install
npm run build
sudo systemctl restart friday
```
