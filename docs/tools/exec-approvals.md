...
- **gateway host** → `friday` process on the gateway machine
...
`~/.friday/exec-approvals.json` directly.

CLI: `friday approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
