---
summary: "Agent tool surface for FRIDAY (browser, canvas, nodes, message, cron) replacing legacy `friday-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `friday-*` skills
---

# Tools (FRIDAY)

FRIDAY exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `friday-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `friday.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, FRIDAY logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`friday node run`).
...
Notes:
- Requires a Brave API key (recommended: `friday configure --section web`, or set `BRAVE_API_KEY`).
...
### `browser`
Control the dedicated friday browser.
...
- Quick smoke: `friday nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `friday gateway` restart in-place)
