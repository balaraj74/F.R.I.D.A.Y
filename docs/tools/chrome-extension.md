---
summary: "Chrome extension: let FRIDAY drive your existing Chrome tab"
read_when:
...
The FRIDAY Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate friday-managed Chrome profile.
...
FRIDAY then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the FRIDAY release (npm package) as static files. There is no separate “build” step.

After upgrading FRIDAY:
- Re-run `friday browser extension install` to refresh the installed files under your FRIDAY state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

FRIDAY ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want FRIDAY to control.
...
- `ON`: attached; FRIDAY can drive that tab.
...
Debugging: `friday sandbox explain`
...
`friday browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `friday browser extension install` first to copy the extension to a stable location under your FRIDAY state directory.
...
- **This is not isolated** like the dedicated friday-managed profile.
