...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FRIDAY
---

# Node + tsx "__name is not a function" crash

## Summary
Running FRIDAY via Node with `tsx` fails at startup with:

```
[friday] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `friday status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch friday.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node friday.mjs status` works on Node 25.
