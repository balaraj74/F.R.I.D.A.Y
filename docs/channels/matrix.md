...
Matrix is an open, decentralized messaging protocol. FRIDAY connects as a Matrix **user**
...
```bash
friday plugins install @friday/matrix
```

Local checkout (when running from a git repo):

```bash
friday plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `friday plugins install @friday/matrix`
   - From a local checkout: `friday plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: FRIDAY calls the same
     login endpoint, stores the access token in `~/.friday/credentials/matrix/credentials.json`,
...
- On first connection, FRIDAY requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  FRIDAY logs a warning.
...
`~/.friday/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `friday pairing list matrix`
  - `friday pairing approve matrix <CODE>`
...
- On startup, FRIDAY resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
