...
Tlon is a decentralized messenger built on Urbit. FRIDAY connects to your Urbit ship and can
...
```bash
friday plugins install @friday/tlon
```

Local checkout (when running from a git repo):

```bash
friday plugins install ./extensions/tlon
```
...
Use these with `friday message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FRIDAY replies in-thread.
