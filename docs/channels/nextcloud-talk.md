...
```bash
friday plugins install @friday/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
friday plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "FRIDAY" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `friday pairing list nextcloud-talk`
  - `friday pairing approve nextcloud-talk <CODE>`
