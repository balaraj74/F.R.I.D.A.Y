...
read_when:
  - Setting up Twitch chat integration for FRIDAY
---
# Twitch (plugin)

Twitch chat support via IRC connection. FRIDAY connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
friday plugins install @friday/twitch
```

Local checkout (when running from a git repo):

```bash
friday plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `FRIDAY_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "friday",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use FRIDAY_TWITCH_ACCESS_TOKEN env var)
...
```bash
FRIDAY_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
