...
## Goals
- Talk to FRIDAY via Discord DMs or guild channels.
...
3. Configure FRIDAY with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `friday pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running FRIDAY in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents FRIDAY needs
...
Discord uses numeric ids everywhere; FRIDAY config prefers ids.
...
### Troubleshooting
- First: run `friday doctor` and `friday channels status --probe` (actionable warnings + quick audits).
...
- On startup, FRIDAY resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror FRIDAY’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; FRIDAY enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`friday gateway --force`) after confirming no other processes own the Discord session.
