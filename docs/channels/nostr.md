...
read_when:
  - You want FRIDAY to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables FRIDAY to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`friday onboard`) and `friday channels add` list optional channel plugins.
...
```bash
friday plugins install @friday/nostr
```

Use a local checkout (dev workflows):

```bash
friday plugins install --link <path-to-friday>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "friday@example.com",
        "lud16": "friday@example.com"
      }
    }
  }
}
