---
summary: "OpenProse: .prose workflows, slash commands, and state in FRIDAY"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In FRIDAY it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
friday plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `friday plugins install ./extensions/open-prose`

Related docs: [Plugins](/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses FRIDAY tools under the hood.
...
## FRIDAY runtime mapping

OpenProse programs map to FRIDAY primitives:

| OpenProse concept | FRIDAY tool |
| --- | --- |
...
Treat `.prose` files like code. Review before running. Use FRIDAY tool allowlists and approval gates to control side effects.
