...
You can also set `FRIDAY_VERBOSE=1` instead of the flag.
...
### `docs.friday.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.friday.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.friday.ai` to the allowlist, then retry.
...
- Check the allowlist: `friday config get agents.defaults.models`
...
If you can, include the relevant log tail from `friday logs --follow`.
