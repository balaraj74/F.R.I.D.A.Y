---
description: Update FRIDAY from upstream when branch has diverged (ahead/behind)
---

# FRIDAY Upstream Sync Workflow
...
# Move old version
mv /Applications/FRIDAY.app /tmp/FRIDAY-backup.app

# Install new build
cp -R dist/FRIDAY.app /Applications/
...
ps aux | grep -E "(friday|gateway)" | grep -v grep

# Test agent functionality by sending a verification message
pnpm friday agent --message "Verification: macOS app rebuild successful - agent is responding." --session-id YOUR_TELEGRAM_SESSION_ID
...
morph-mcp_warpgrep_codebase_search search_string="Find deprecated FileManager.default and Thread.isMainThread usages in Swift files" repo_path="/Volumes/Main SSD/Developer/fridayis"
morph-mcp_warpgrep_codebase_search search_string="Locate Peekaboo submodule and macOS app Swift files with concurrency issues" repo_path="/Volumes/Main SSD/Developer/fridayis"
```
...
cd /Volumes/Main SSD/Developer/fridayis
...
# Update friday.json with fallback chains
...
morph-mcp_warpgrep_codebase_search search_string="Find all Swift files using deprecated FileManager.default or Thread.isMainThread" repo_path="/Volumes/Main SSD/Developer/fridayis"
```
...
pnpm friday agent --message "Verification: Upstream sync and macOS rebuild completed successfully." --session-id YOUR_TELEGRAM_SESSION_ID || echo "Warning: Agent test failed - check Telegram for verification message"
