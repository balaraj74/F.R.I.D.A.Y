...
This document describes how the FRIDAY ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`friday acp` exposes an ACP agent over stdio and forwards prompts to a running
FRIDAY Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a FRIDAY Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `friday acp` over stdio.

Example config:

```bash
friday config set gateway.remote.url wss://gateway-host:18789
friday config set gateway.remote.token <token>
```

Example run:

```bash
friday acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
friday acp --session agent:main:main
friday acp --session agent:design:main
friday acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “FRIDAY ACP” to start a thread.

## Execution Model

- ACP client spawns `friday acp` and speaks ACP messages over stdio.
...
```bash
friday acp --session agent:main:main
friday acp --session-label "support inbox"
friday acp --reset-session
```
...
`friday acp` resolves the Gateway URL and auth from CLI flags or config:
