...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [FRIDAYCapability.canvas.rawValue, FRIDAYCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(FRIDAYCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(FRIDAYCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = FRIDAYLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(FRIDAYCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            FRIDAYSystemCommand.notify.rawValue,
            FRIDAYSystemCommand.which.rawValue,
            FRIDAYSystemCommand.run.rawValue,
            FRIDAYSystemCommand.execApprovalsGet.rawValue,
            FRIDAYSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(FRIDAYCapability.camera.rawValue) {
            commands.append(FRIDAYCameraCommand.list.rawValue)
            commands.append(FRIDAYCameraCommand.snap.rawValue)
            commands.append(FRIDAYCameraCommand.clip.rawValue)
        }
        if caps.contains(FRIDAYCapability.location.rawValue) {
            commands.append(FRIDAYLocationCommand.get.rawValue)
        }
