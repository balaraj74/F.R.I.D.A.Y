...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = FRIDAYCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = FRIDAYCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = FRIDAYCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = FRIDAYCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = FRIDAYCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__friday__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: FRIDAYLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for friday:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case FRIDAYLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case FRIDAYCanvasCommand.present.rawValue,
                 FRIDAYCanvasCommand.hide.rawValue,
                 FRIDAYCanvasCommand.navigate.rawValue,
                 FRIDAYCanvasCommand.evalJS.rawValue,
                 FRIDAYCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case FRIDAYCanvasA2UICommand.reset.rawValue,
                 FRIDAYCanvasA2UICommand.push.rawValue,
                 FRIDAYCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case FRIDAYCameraCommand.list.rawValue,
                 FRIDAYCameraCommand.snap.rawValue,
                 FRIDAYCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case FRIDAYScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: FRIDAYNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FRIDAYNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(FRIDAYLocationGetParams.self, from: req.paramsJSON)) ??
            FRIDAYLocationGetParams()
        ...
        let payload = FRIDAYLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case FRIDAYCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(FRIDAYCanvasPresentParams.self, from: req.paramsJSON)) ??
                FRIDAYCanvasPresentParams()
        ...
        case FRIDAYCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case FRIDAYCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(FRIDAYCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case FRIDAYCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(FRIDAYCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FRIDAYCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(FRIDAYCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case FRIDAYCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.fridayA2UI) return JSON.stringify({ ok: false, error: "missing fridayA2UI" });
              return JSON.stringify(globalThis.fridayA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FRIDAYCanvasA2UICommand.push.rawValue, FRIDAYCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == FRIDAYCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(FRIDAYCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try FRIDAYCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(FRIDAYCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try FRIDAYCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.fridayA2UI) return JSON.stringify({ ok: false, error: "missing fridayA2UI" });
                const messages = \(messagesJSON);
                return JSON.stringify(globalThis.fridayA2UI.applyMessages(messages));
              } catch (e) {
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case FRIDAYCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FRIDAYCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(FRIDAYCameraSnapParams.self, from: req.paramsJSON)) ??
                FRIDAYCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case FRIDAYCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(FRIDAYCameraClipParams.self, from: req.paramsJSON)) ??
                FRIDAYCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(FRIDAYScreenRecordParams.self, from: req.paramsJSON)) ??
            FRIDAYScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> FRIDAYLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return FRIDAYLocationMode(rawValue: raw) ?? .off
    }
