...
struct IOSGatewayChatTransport: FRIDAYChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> FRIDAYChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(FRIDAYChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> FRIDAYChatHistoryPayload {
        ...
        return try JSONDecoder().decode(FRIDAYChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [FRIDAYChatAttachmentPayload]) async throws -> FRIDAYChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [FRIDAYChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(FRIDAYChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(FRIDAYGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FRIDAYGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FRIDAYChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: FRIDAYAgentEventPayload.self)
                        {
