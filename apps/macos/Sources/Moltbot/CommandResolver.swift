...
enum CommandResolver {
    private static let projectRootDefaultsKey = "friday.gatewayProjectRootPath"
    private static let helperName = "friday"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/friday.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/friday")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let fridayPaths = self.fridayManagedPaths(home: home)
        if !fridayPaths.isEmpty {
            extras.insert(contentsOf: fridayPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + fridayPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func fridayManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".friday")
    ...
    static func fridayExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectFRIDAYExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/friday.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyFRIDAYInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.fridayExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let fridayPath = self.projectFRIDAYExecutable(projectRoot: root) {
                return [fridayPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "friday", subcommand] + extraArgs
            }
            if let fridayPath = self.fridayExecutable(searchPaths: searchPaths) {
                return [fridayPath, subcommand] + extraArgs
            }

            let missingEntry = """
            friday entrypoint missing (looked for dist/index.js or bin/friday.js); run pnpm build.
            """
    ...
    // Existing callers still refer to fridayCommand; keep it as node alias.
    static func fridayCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real friday CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/friday"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/friday.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/friday.js"
            node "$PRJ/bin/friday.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent friday"
          pnpm --silent friday \(quotedArgs);
        else
          echo "friday CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? FRIDAYConfigFile.loadDict()
