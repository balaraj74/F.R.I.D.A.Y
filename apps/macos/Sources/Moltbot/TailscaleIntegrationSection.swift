...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.friday/friday.json. Prefer FRIDAY_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.friday/friday.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.friday/friday.json. Restart the gateway to apply."
        }
