...
struct MacGatewayChatTransport: FRIDAYChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> FRIDAYChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(FRIDAYChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> FRIDAYChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                FRIDAYGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<FRIDAYChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any FRIDAYChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = FRIDAYChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: FRIDAYChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<FRIDAYChatView>) -> NSViewController
    {
