...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FRIDAYSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FRIDAYSessionPreviewEntry,
    ...
    private static func cache(payload: FRIDAYSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FRIDAYChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FRIDAYKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FRIDAYKit.AnyCodable]) -> [FRIDAYChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FRIDAYChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FRIDAYChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FRIDAYChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FRIDAYChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FRIDAYChatMessage) -> String? {
