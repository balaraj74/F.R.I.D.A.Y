...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: FRIDAYProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: FRIDAYKit.AnyCodable]()) {
                $0[$1.key] = FRIDAYKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or FRIDAY_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: FRIDAYProtocol.AnyCodable?) -> [String: FRIDAYProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: FRIDAYProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: FRIDAYKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: FRIDAYProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("friday.control.heartbeat")
    static let controlAgentEvent = Notification.Name("friday.control.agent")
}
