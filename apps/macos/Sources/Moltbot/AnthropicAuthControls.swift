...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: FRIDAYOAuthStore.AnthropicOAuthStatus = FRIDAYOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: FRIDAYOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = FRIDAYOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = FRIDAYOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try FRIDAYOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. FRIDAY can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: FRIDAYOAuthStore.AnthropicOAuthStatus,
