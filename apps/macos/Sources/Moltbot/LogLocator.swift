...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["FRIDAY_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/friday")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("friday-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("friday-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/friday/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("friday") && $0.pathExtension == "log" }
            .max { lhs, rhs in
