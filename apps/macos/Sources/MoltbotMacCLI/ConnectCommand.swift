...
struct ConnectOptions {
    ...
    var clientId: String = "friday-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          friday-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: friday-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "FRIDAY macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("FRIDAY macOS Gateway Connect")
...
private func resolvedToken(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let token = ProcessInfo.processInfo.environment["FRIDAY_GATEWAY_TOKEN"], !token.isEmpty {
        return token
    }
    if mode == "remote" {
...
private func resolvedPassword(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let password = ProcessInfo.processInfo.environment["FRIDAY_GATEWAY_PASSWORD"], !password.isEmpty {
        return password
    }
    if mode == "remote" {
