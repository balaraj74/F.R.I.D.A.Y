...
func runWizardCommand(_ args: [String]) async {
    ...
    if opts.help {
        ...
        Usage:
          friday-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
...
actor GatewayWizardClient {
    ...
    private func sendConnect() async throws {
        ...
        let clientId = "friday-macos"
        let clientMode = "ui"
        let role = "operator"
        let scopes: [String] = []
        let client: [String: ProtoAnyCodable] = [
            "id": ProtoAnyCodable(clientId),
            "displayName": ProtoAnyCodable(Host.current().localizedName ?? "FRIDAY macOS Wizard CLI"),
