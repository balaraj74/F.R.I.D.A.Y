...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: FRIDAYProtocol.AnyCodable(["ok": FRIDAYProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: FRIDAYProtocol.AnyCodable(["ok": FRIDAYProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = FRIDAYProtocol.AnyCodable([
            "runId": FRIDAYProtocol.AnyCodable("run-1"),
            "sessionKey": FRIDAYProtocol.AnyCodable("main"),
            "state": FRIDAYProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: FRIDAYProtocol.AnyCodable(["a": FRIDAYProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
