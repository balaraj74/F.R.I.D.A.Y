...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: FRIDAYProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: FRIDAYProtocol.AnyCodable] = [
            "presence": .init([FRIDAYProtocol.AnyCodable(entry)]),
        ]
        let payload = FRIDAYProtocol.AnyCodable(payloadMap)
