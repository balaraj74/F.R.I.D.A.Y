...
import "@a2ui/lit/ui";
import { themeContext } from "@friday/a2ui-theme-context";
...
class FRIDAYA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--friday-a2ui-inset-top, 0px)
        var(--friday-a2ui-inset-right, 0px)
        var(--friday-a2ui-inset-bottom, 0px)
        var(--friday-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--friday-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--friday-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--friday-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--friday-a2ui-empty-top, var(--friday-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.fridayA2UI = api;
    globalThis.fridayA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    globalThis.addEventListener("friday:a2ui-action-status", this.#statusListener);
    this.#syncSurfaces();
  }
  ...
  disconnectedCallback() {
    ...
    if (this.#statusListener) {
      globalThis.removeEventListener("friday:a2ui-action-status", this.#statusListener);
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__fridayLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.fridayCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.fridayCanvasA2UIAction ??
      globalThis.fridayCanvasA2UIAction ??
      globalThis.fridayCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (
          handler === globalThis.fridayCanvasA2UIAction ||
          handler === globalThis.fridayCanvasA2UIAction
        ) {
...
customElements.define("friday-a2ui-host", FRIDAYA2UIHost);
