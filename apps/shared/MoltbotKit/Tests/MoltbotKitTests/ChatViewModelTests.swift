...
private final class TestChatTransport: @unchecked Sendable, FRIDAYChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [FRIDAYChatHistoryPayload]
    private let sessionsResponses: [FRIDAYChatSessionsListResponse]

    private let stream: AsyncStream<FRIDAYChatTransportEvent>
    private let continuation: AsyncStream<FRIDAYChatTransportEvent>.Continuation

    init(
        historyResponses: [FRIDAYChatHistoryPayload],
        sessionsResponses: [FRIDAYChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<FRIDAYChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> FRIDAYChatHistoryPayload {
        ...
        return self.historyResponses.last ?? FRIDAYChatHistoryPayload(
        ...
        attachments _: [FRIDAYChatAttachmentPayload]) async throws -> FRIDAYChatSendResponse
    {
        ...
        return FRIDAYChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> FRIDAYChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? FRIDAYChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: FRIDAYChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = FRIDAYChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = FRIDAYChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { FRIDAYChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = FRIDAYChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = FRIDAYChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = FRIDAYChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { FRIDAYChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
