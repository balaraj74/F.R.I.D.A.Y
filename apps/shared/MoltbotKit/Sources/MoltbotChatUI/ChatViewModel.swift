...
private let chatUILogger = Logger(subsystem: "bot.molt", category: "FRIDAYChatUI")

@MainActor
@Observable
public final class FRIDAYChatViewModel {
    public private(set) var messages: [FRIDAYChatMessage] = []
    ...
    public var attachments: [FRIDAYPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [FRIDAYChatPendingToolCall] = []
    public private(set) var sessions: [FRIDAYChatSessionEntry] = []
    private let transport: any FRIDAYChatTransport
    ...
    private var pendingToolCallsById: [String: FRIDAYChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any FRIDAYChatTransport) {
    ...
    public var sessionChoices: [FRIDAYChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [FRIDAYChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [FRIDAYChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: FRIDAYPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [FRIDAYChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: FRIDAYChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [FRIDAYChatMessage]) -> [FRIDAYChatMessage] {
        var result: [FRIDAYChatMessage] = []
    ...
    private static func dedupeKey(for message: FRIDAYChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [FRIDAYChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> FRIDAYChatAttachmentPayload in
            FRIDAYChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> FRIDAYChatSessionEntry {
        FRIDAYChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: FRIDAYChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: FRIDAYChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: FRIDAYAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = FRIDAYChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> FRIDAYPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
