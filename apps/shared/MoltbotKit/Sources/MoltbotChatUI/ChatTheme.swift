...
enum FRIDAYChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FRIDAYChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("FRIDAYChatTheme.onboardingAssistantBubble"),
...
enum FRIDAYPlatformImageFactory {
    static func image(_ image: FRIDAYPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
