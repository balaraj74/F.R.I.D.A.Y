...
import type { FRIDAYApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as FRIDAYApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as FRIDAYApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as FRIDAYApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as FRIDAYApp);
    await loadDevices(host as unknown as FRIDAYApp);
    await loadConfig(host as unknown as FRIDAYApp);
    await loadExecApprovals(host as unknown as FRIDAYApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as FRIDAYApp);
    await loadConfig(host as unknown as FRIDAYApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as FRIDAYApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as FRIDAYApp, { reset: true });
    scheduleLogsScroll(
...
export function inferBasePath() {
  ...
  const configured = window.__FRIDAY_CONTROL_UI_BASE_PATH__;
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as FRIDAYApp, false),
    loadPresence(host as unknown as FRIDAYApp),
    loadSessions(host as unknown as FRIDAYApp),
    loadCronStatus(host as unknown as FRIDAYApp),
    loadDebug(host as unknown as FRIDAYApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as FRIDAYApp, true),
    loadConfigSchema(host as unknown as FRIDAYApp),
    loadConfig(host as unknown as FRIDAYApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as FRIDAYApp, false),
    loadCronStatus(host as unknown as FRIDAYApp),
    loadCronJobs(host as unknown as FRIDAYApp),
  ]);
}
