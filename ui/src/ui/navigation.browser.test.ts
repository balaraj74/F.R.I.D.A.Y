...
const originalConnect = FRIDAYApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("friday-app") as FRIDAYApp;
...
beforeEach(() => {
  FRIDAYApp.prototype.connect = () => {
    // no-op: avoid real gateway WS connections in browser tests
  };
  window.__FRIDAY_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});

afterEach(() => {
  FRIDAYApp.prototype.connect = originalConnect;
  window.__FRIDAY_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/friday/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/friday");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/friday/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__FRIDAY_CONTROL_UI_BASE_PATH__ = "/friday";
    const app = mountApp("/friday/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/friday");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/friday/sessions");
  });
