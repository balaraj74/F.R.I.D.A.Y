...
import type { FRIDAYConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("friday-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("friday-bin-sherpa-");
      const modelDir = await makeTempDir("friday-sherpa-model-");
      ...
      const cfg: FRIDAYConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("friday-bin-whispercpp-");
      const modelDir = await makeTempDir("friday-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("friday-bin-gemini-");
      ...
      const cfg: FRIDAYConfig = { tools: { media: { image: {} } } };
