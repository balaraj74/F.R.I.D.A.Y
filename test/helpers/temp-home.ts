...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("FRIDAY_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.FRIDAY_STATE_DIR = path.join(base, ".friday");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "friday-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".friday", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
