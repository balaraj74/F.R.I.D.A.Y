...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `friday-e2e-${name}-`));
  const configDir = path.join(homeDir, ".friday");
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        FRIDAY_GATEWAY_TOKEN: inst.gatewayToken,
        FRIDAY_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          FRIDAY_GATEWAY_PORT: String(gwA.port),
          FRIDAY_GATEWAY_TOKEN: gwA.gatewayToken,
          FRIDAY_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          FRIDAY_GATEWAY_PORT: String(gwB.port),
          FRIDAY_GATEWAY_TOKEN: gwB.gatewayToken,
          FRIDAY_GATEWAY_PASSWORD: "",
        }),
