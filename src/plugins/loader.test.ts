...
import { loadFRIDAYPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.FRIDAY_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `friday-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "friday.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.FRIDAY_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadFRIDAYPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.FRIDAY_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadFRIDAYPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadFRIDAYPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
