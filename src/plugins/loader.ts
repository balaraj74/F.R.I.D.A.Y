...
import { discoverFRIDAYPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FRIDAYPluginDefinition;
  register?: FRIDAYPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as FRIDAYPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as FRIDAYPluginDefinition;
...
export function loadFRIDAYPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverFRIDAYPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const jiti = createJiti(import.meta.url, {
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            "friday/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: FRIDAYPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as FRIDAYPluginModule;
    } catch (err) {
