...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.FRIDAY_GATEWAY_PORT).toBe("18789");
    expect(env.FRIDAY_GATEWAY_TOKEN).toBe("secret");
    expect(env.FRIDAY_SERVICE_MARKER).toBe("friday");
    expect(env.FRIDAY_SERVICE_KIND).toBe("gateway");
    expect(typeof env.FRIDAY_SERVICE_VERSION).toBe("string");
    expect(env.FRIDAY_SYSTEMD_UNIT).toBe("friday-gateway.service");
    if (process.platform === "darwin") {
      expect(env.FRIDAY_LAUNCHD_LABEL).toBe("bot.molt.gateway");
    }
  });

  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", FRIDAY_PROFILE: "work" },
      port: 18789,
    });
    expect(env.FRIDAY_SYSTEMD_UNIT).toBe("friday-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.FRIDAY_LAUNCHD_LABEL).toBe("bot.molt.work");
    }
  });
});
