...
async function withLaunchctlStub(
  ...
  const originalLogPath = process.env.FRIDAY_TEST_LAUNCHCTL_LOG;
  const originalListOutput = process.env.FRIDAY_TEST_LAUNCHCTL_LIST_OUTPUT;

  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-launchctl-test-"));
  try {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const logPath = process.env.FRIDAY_TEST_LAUNCHCTL_LOG;",
        "if (logPath) {",
        '  fs.appendFileSync(logPath, JSON.stringify(args) + "\\n", "utf8");',
        "}",
        'if (args[0] === "list") {',
        '  const output = process.env.FRIDAY_TEST_LAUNCHCTL_LIST_OUTPUT || "";',
    ...
    process.env.FRIDAY_TEST_LAUNCHCTL_LOG = logPath;
    process.env.FRIDAY_TEST_LAUNCHCTL_LIST_OUTPUT = options.listOutput ?? "";
    process.env.PATH = `${binDir}${path.delimiter}${originalPath ?? ""}`;

    await run({
      env: {
        HOME: homeDir,
        FRIDAY_PROFILE: "default",
      },
      logPath,
    });
  } finally {
    ...
    if (originalLogPath === undefined) {
      delete process.env.FRIDAY_TEST_LAUNCHCTL_LOG;
    } else {
      process.env.FRIDAY_TEST_LAUNCHCTL_LOG = originalLogPath;
    }
    if (originalListOutput === undefined) {
      delete process.env.FRIDAY_TEST_LAUNCHCTL_LIST_OUTPUT;
    } else {
      process.env.FRIDAY_TEST_LAUNCHCTL_LIST_OUTPUT = originalListOutput;
    }
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when FRIDAY_PROFILE is default", () => {
  ...
  it("uses default label when FRIDAY_PROFILE is unset", () => {
  ...
  it("uses profile-specific label when FRIDAY_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", FRIDAY_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.molt.jbphoenix.plist",
    );
  });

  it("prefers FRIDAY_LAUNCHD_LABEL over FRIDAY_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      FRIDAY_PROFILE: "jbphoenix",
      FRIDAY_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from FRIDAY_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      FRIDAY_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty FRIDAY_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      FRIDAY_PROFILE: "myprofile",
  ...
  it("trims whitespace from FRIDAY_PROFILE", () => {
    const env = { HOME: "/Users/test", FRIDAY_PROFILE: "  myprofile  " };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.molt.myprofile.plist",
    );
  });
