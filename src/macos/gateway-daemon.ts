...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `friday --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.FRIDAY_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ?? process.env.FRIDAY_GATEWAY_BIND ?? cfg.gateway?.bind ?? "loopback";
  const bind =
    bindRaw === "loopback" ||
    bindRaw === "lan" ||
    bindRaw === "auto" ||
