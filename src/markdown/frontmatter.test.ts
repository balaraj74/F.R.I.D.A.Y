...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { friday?: { emoji?: string } };
    expect(parsed.friday?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { friday?: { events?: string[] } };
