...
export function listSignalAccountIds(cfg: FRIDAYConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: FRIDAYConfig): string {
...
function mergeSignalAccountConfig(cfg: FRIDAYConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: FRIDAYConfig): ResolvedSignalAccount[] {
