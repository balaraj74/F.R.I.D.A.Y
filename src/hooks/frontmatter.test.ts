...
`;
    ...
    expect(parsed.friday.emoji).toBe("💾");
    expect(parsed.friday.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.friday.emoji).toBe("📝");
    expect(parsed.friday.events).toEqual(["command"]);
    expect(parsed.friday.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.friday.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"friday": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"friday": {"events": ["test"]}}');
  });
...
describe("resolveFRIDAYMetadata", () => {
  it("extracts friday metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveFRIDAYMetadata(frontmatter);
  ...
  it("returns undefined when friday key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        friday: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with FRIDAY" },
            { id: "npm", kind: "npm", package: "@friday/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@friday/hook");
  });
...
`;
    ...
    const friday = resolveFRIDAYMetadata(frontmatter);
    expect(friday).toBeDefined();
    expect(friday?.emoji).toBe("💾");
    expect(friday?.events).toEqual(["command:new"]);
    expect(friday?.requires?.config).toEqual(["workspace.dir"]);
    expect(friday?.install?.[0].kind).toBe("bundled");
  });
