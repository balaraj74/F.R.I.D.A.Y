...
import type { FRIDAYPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: FRIDAYPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: FRIDAYPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
