...
This directory contains hooks that ship with FRIDAY. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.friday.ai/hooks/soul-evil
...
metadata:
  { "friday": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.friday/friday.json`:
...
2. Restart gateway: `pkill -9 -f 'friday.*gateway' && pnpm friday gateway`
3. Enable the hook: `friday hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.friday.ai/hooks
