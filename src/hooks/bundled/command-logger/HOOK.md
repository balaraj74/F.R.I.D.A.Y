...
homepage: https://docs.friday.ai/hooks#command-logger
metadata:
  {
    "friday":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.friday/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.friday/logs/commands.log ~/.friday/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/friday`:
   ```
   /home/username/.friday/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
