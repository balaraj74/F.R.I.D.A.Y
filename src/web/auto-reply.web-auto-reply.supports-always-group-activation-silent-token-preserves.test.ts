...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "friday-web-home-"));
...
const makeSessionStore = async (
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-session-"));
...
describe("web auto-reply", () => {
  ...
  it("supports always-on group activation with silent token and clears pending history", async () => {
    ...
    setLoadConfigMock(() => ({
      messages: {
        groupChat: { mentionPatterns: ["@friday"] },
      },
  ...
  it("ignores JID mentions in self-chat mode (group chats)", async () => {
    ...
    // Text-based mentionPatterns still work (user can type "friday" explicitly).
    await capturedOnMessage?.({
      body: "friday ping",
      from: "123@g.us",
      conversationId: "123@g.us",
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/friday-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/friday-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
