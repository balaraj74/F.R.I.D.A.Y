...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "friday-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".friday", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<FRIDAYConfig> = {},
): FRIDAYConfig {
  const base: FRIDAYConfig = {
    agents: {
      defaults: {
        model: "anthropic/claude-opus-4-5",
        workspace: path.join(home, "friday"),
      },
    },
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".friday", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
