...
export function listSlackAccountIds(cfg: FRIDAYConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: FRIDAYConfig): string {
...
function mergeSlackAccountConfig(cfg: FRIDAYConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: FRIDAYConfig): ResolvedSlackAccount[] {
