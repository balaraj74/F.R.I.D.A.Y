...
export async function runDaemonInstall(opts: DaemonInstallOptions) {
  ...
  if (loaded) {
    if (!opts.force) {
      ...
      if (!json) {
        ...
        defaultRuntime.log(`Reinstall with: ${formatCliCommand("friday gateway install --force")}`);
      }
      return;
    }
  }
  ...
  const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
    env: process.env,
    port,
    token: opts.token || cfg.gateway?.auth?.token || process.env.FRIDAY_GATEWAY_TOKEN,
    runtime: runtimeRaw,
    warn: (message) => {
