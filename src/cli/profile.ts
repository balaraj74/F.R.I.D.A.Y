...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.friday${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.FRIDAY_PROFILE = profile;

  const stateDir = env.FRIDAY_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.FRIDAY_STATE_DIR?.trim()) env.FRIDAY_STATE_DIR = stateDir;

  if (!env.FRIDAY_CONFIG_PATH?.trim()) {
    env.FRIDAY_CONFIG_PATH = path.join(stateDir, "friday.json");
  }

  if (profile === "dev" && !env.FRIDAY_GATEWAY_PORT?.trim()) {
    env.FRIDAY_GATEWAY_PORT = "19001";
  }
}
