...
const STEP_LABELS: Record<string, string> = {
  ...
  "friday doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "friday";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "friday"]);
const CLI_NAME = resolveCliName();
const FRIDAY_REPO_URL = "https://github.com/friday/friday.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "friday");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("friday@")) return trimmed.slice("friday@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.FRIDAY_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", FRIDAY_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `FRIDAY_GIT_DIR points at a non-git directory: ${params.dir}. Set FRIDAY_GIT_DIR to an empty folder or a friday checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`FRIDAY_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveFRIDAYPackageRoot({
  ...
  defaultRuntime.log(theme.heading("FRIDAY update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating FRIDAY..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this FRIDAY install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("friday doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("friday gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g friday@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g friday@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.FRIDAY_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.FRIDAY_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("friday gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("friday doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("friday gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("friday gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `friday update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `FRIDAY_GIT_DIR points at a non-git directory: ${gitDir}. Set FRIDAY_GIT_DIR to an empty folder or a friday checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via FRIDAY_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update FRIDAY to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["friday update", "Update a source checkout (git)"],
        ["friday update --channel beta", "Switch to beta channel (git + npm)"],
        ["friday update --channel dev", "Switch to dev channel (git + npm)"],
        ["friday update --tag beta", "One-off update to a dist-tag or version"],
        ["friday update --no-restart", "Update without restarting the service"],
        ["friday update --json", "Output result as JSON"],
        ["friday update --yes", "Non-interactive (accept downgrade prompts)"],
        ["friday update wizard", "Interactive update wizard"],
        ["friday --update", "Shorthand for friday update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run friday update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.friday.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["friday update status", "Show channel + version status."],
          ["friday update status --json", "JSON output."],
          ["friday update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.friday.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
