...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    FRIDAY_CONFIG_PATH: process.env.FRIDAY_CONFIG_PATH,
    FRIDAY_GATEWAY_PORT: process.env.FRIDAY_GATEWAY_PORT,
    FRIDAY_PROFILE: process.env.FRIDAY_PROFILE,
  };

  beforeEach(() => {
    process.env.FRIDAY_STATE_DIR = "/tmp/friday-cli-state";
    process.env.FRIDAY_CONFIG_PATH = "/tmp/friday-cli-state/friday.json";
    delete process.env.FRIDAY_GATEWAY_PORT;
    delete process.env.FRIDAY_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.FRIDAY_STATE_DIR !== undefined)
      process.env.FRIDAY_STATE_DIR = originalEnv.FRIDAY_STATE_DIR;
    else delete process.env.FRIDAY_STATE_DIR;

    if (originalEnv.FRIDAY_CONFIG_PATH !== undefined)
      process.env.FRIDAY_CONFIG_PATH = originalEnv.FRIDAY_CONFIG_PATH;
    else delete process.env.FRIDAY_CONFIG_PATH;

    if (originalEnv.FRIDAY_GATEWAY_PORT !== undefined)
      process.env.FRIDAY_GATEWAY_PORT = originalEnv.FRIDAY_GATEWAY_PORT;
    else delete process.env.FRIDAY_GATEWAY_PORT;

    if (originalEnv.FRIDAY_PROFILE !== undefined)
      process.env.FRIDAY_PROFILE = originalEnv.FRIDAY_PROFILE;
    else delete process.env.FRIDAY_PROFILE;
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        FRIDAY_PROFILE: "dev",
        FRIDAY_STATE_DIR: "/tmp/friday-daemon-state",
        FRIDAY_CONFIG_PATH: "/tmp/friday-daemon-state/friday.json",
        FRIDAY_GATEWAY_PORT: "19001",
      },
