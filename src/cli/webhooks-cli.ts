...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.friday.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + FRIDAY hooks")
    ...
    .option("--hook-url <url>", "FRIDAY hook URL")
    .option("--hook-token <token>", "FRIDAY hook token")
