...
${formatHelpExamples([
  ['friday agent --to +15555550123 --message "status update"', "Start a new session."],
  ['friday agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'friday agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'friday agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['friday agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'friday agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.friday.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.friday.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['friday agents set-identity --agent main --name "Clawd" --emoji "🤖"', "Set name + emoji."],
  ["friday agents set-identity --agent main --avatar avatars/friday.png", "Set avatar path."],
  ["friday agents set-identity --workspace ~/friday --from-identity", "Load from IDENTITY.md."],
  [
    "friday agents set-identity --identity-file ~/friday/IDENTITY.md --agent main",
