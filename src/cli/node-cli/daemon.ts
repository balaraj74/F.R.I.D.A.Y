...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("friday node install"), formatCliCommand("friday node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("friday node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    FRIDAY_LOG_PREFIX: baseEnv.FRIDAY_LOG_PREFIX ?? "node",
