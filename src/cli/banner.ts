...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const title = cliName === "friday" ? "🤖 FRIDAY" : "🤖 FRIDAY";
  const prefix = "🤖 ";
...
export function formatCliBannerArt(options: BannerOptions = {}): string {
  const rich = options.richTty ?? isRich();
  if (!rich) return FRIDAY_ASCII.join("\n");
  ...
  const colored = FRIDAY_ASCII.map((line) => {
    if (line.includes("Personal AI Assistant")) {
      return (
        theme.muted("      ") +
        theme.accent("🤖") +
        theme.info(" Your Personal AI Assistant ") +
        theme.accent("🤖")
      );
    }
