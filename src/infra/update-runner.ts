...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "friday-update-preflight-"));
    ...
    const doctorStep = await runStep(
      step(
        "friday doctor",
        managerScriptArgs(manager, "friday", ["doctor", "--non-interactive"]),
        gitRoot,
        { FRIDAY_UPDATE_IN_PROGRESS: "1" },
      ),
    );
