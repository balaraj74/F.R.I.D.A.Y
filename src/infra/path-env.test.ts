...
import { ensureFRIDAYCliOnPath } from "./path-env.js";

describe("ensureFRIDAYCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling friday exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "friday-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "friday");
      await fs.writeFile(cliPath, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(cliPath, 0o755);

      const originalPath = process.env.PATH;
      const originalFlag = process.env.FRIDAY_PATH_BOOTSTRAPPED;
      process.env.PATH = "/usr/bin";
      delete process.env.FRIDAY_PATH_BOOTSTRAPPED;
      try {
        ensureFRIDAYCliOnPath({
          execPath: cliPath,
          cwd: tmp,
          homeDir: tmp,
          platform: "darwin",
        ...
        if (originalFlag === undefined) delete process.env.FRIDAY_PATH_BOOTSTRAPPED;
        else process.env.FRIDAY_PATH_BOOTSTRAPPED = originalFlag;
      }
    } finally {
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "friday");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "friday");
