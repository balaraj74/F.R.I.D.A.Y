...
export function triggerFRIDAYRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.FRIDAY_SYSTEMD_UNIT,
        process.env.FRIDAY_PROFILE,
      );
  ...
  const label =
    process.env.FRIDAY_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.FRIDAY_PROFILE);
