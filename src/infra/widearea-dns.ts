...
export const WIDE_AREA_DISCOVERY_DOMAIN = "friday.internal.";
export const WIDE_AREA_ZONE_FILENAME = "friday.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*friday-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "friday";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "friday");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "friday-gw");
  ...
  records.push(`_friday-gw._tcp IN PTR ${instanceLabel}._friday-gw._tcp`);
  records.push(`${instanceLabel}._friday-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._friday-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; friday-content-hash: ${contentHash}\n${contentBody}`;
}
