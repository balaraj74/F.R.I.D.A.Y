...
function candidateBinDirs(opts: EnsureFRIDAYPathOpts): string[] {
  ...
  // Bundled macOS app: `friday` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingFRIDAY = path.join(execDir, "friday");
    if (isExecutable(siblingFRIDAY)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/friday` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "friday"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `friday` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFRIDAYCliOnPath(opts: EnsureFRIDAYPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FRIDAY_PATH_BOOTSTRAPPED)) return;
  process.env.FRIDAY_PATH_BOOTSTRAPPED = "1";
