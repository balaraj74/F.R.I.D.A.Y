...
type AgentConfig = Partial<NonNullable<NonNullable<FRIDAYConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.friday/sessions/<SessionId>.jsonl)
...
const formatVoiceModeLine = (config?: FRIDAYConfig, sessionEntry?: SessionEntry): string | null => {
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🤖 FRIDAY ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: FRIDAYConfig): string {
  const lines = ["ℹ️ Help", ""];
