...
import { scheduleGatewaySigusr1Restart, triggerFRIDAYRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting friday in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerFRIDAYRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting friday via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
