...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-commands-"));
...
function buildParams(commandBody: string, cfg: FRIDAYConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
