import type { BrowserConfig, BrowserProfileConfig, FRIDAYConfig } from "../config/config.js";
import {
...
/**
 * Ensure the default "friday" profile exists in the profiles map.
 ...
 *
 * Note: this is a FRIDAY browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for friday CDP).
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "friday";
