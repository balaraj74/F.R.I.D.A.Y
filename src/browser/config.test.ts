...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const friday = resolveProfile(resolved, "friday");
    expect(friday?.driver).toBe("friday");
    expect(friday?.cdpPort).toBe(18800);
    expect(friday?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from FRIDAY_GATEWAY_PORT when unset", () => {
    const prev = process.env.FRIDAY_GATEWAY_PORT;
    process.env.FRIDAY_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const friday = resolveProfile(resolved, "friday");
      expect(friday?.cdpPort).toBe(19012);
      expect(friday?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.FRIDAY_GATEWAY_PORT;
      } else {
        process.env.FRIDAY_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const friday = resolveProfile(resolved, "friday");
      expect(friday?.cdpPort).toBe(19022);
      expect(friday?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.FRIDAY_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "friday");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        friday: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("friday");
  });
});
