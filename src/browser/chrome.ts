...
export async function launchClawdChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🤖 friday browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`friday browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`friday browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🤖 friday browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
