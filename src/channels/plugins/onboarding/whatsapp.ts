...
function setWhatsAppDmPolicy(cfg: FRIDAYConfig, dmPolicy: DmPolicy): FRIDAYConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: FRIDAYConfig, allowFrom?: string[]): FRIDAYConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: FRIDAYConfig, selfChatMode: boolean): FRIDAYConfig {
...
async function detectWhatsAppLinked(cfg: FRIDAYConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so FRIDAY can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for FRIDAY" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("friday channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
