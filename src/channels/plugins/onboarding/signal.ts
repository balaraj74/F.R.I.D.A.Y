...
function setSignalDmPolicy(cfg: FRIDAYConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
export const signalOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    await prompter.note(
      [
        'Link device with: signal-cli link -n "FRIDAY"',
        "Scan QR in Signal → Linked Devices",
        `Then run: ${formatCliCommand("friday gateway call channels.status --params '{\"probe\":true}'")}`,
        `Docs: ${formatDocsLink("/signal", "signal")}`,
      ].join("\n"),
      "Signal next steps",
    );
