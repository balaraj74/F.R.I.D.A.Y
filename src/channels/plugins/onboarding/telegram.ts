...
function setTelegramDmPolicy(cfg: FRIDAYConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("friday logs --follow")}\` (safest)`,
