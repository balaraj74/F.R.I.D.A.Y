...
function setSlackDmPolicy(cfg: FRIDAYConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "FRIDAY";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for FRIDAY`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: FRIDAYConfig, allowFrom: string[]): FRIDAYConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
