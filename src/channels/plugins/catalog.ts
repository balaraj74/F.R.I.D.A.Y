...
import { discoverFRIDAYPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { FRIDAYPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  friday?: FRIDAYPackageManifest;
  [LEGACY_MANIFEST_KEY]?: FRIDAYPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["FRIDAY_PLUGIN_CATALOG_PATHS", "FRIDAY_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<FRIDAYPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageFRIDAY?: FRIDAYPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageFRIDAY;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.friday ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageFRIDAY: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverFRIDAYPlugins({ workspaceDir: options.workspaceDir });
