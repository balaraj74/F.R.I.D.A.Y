...
function resolveDefaultAccountId(cfg: FRIDAYConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: FRIDAYConfig): boolean {
...
export function summarizeBindings(cfg: FRIDAYConfig, bindings: AgentBinding[]): string[] {
