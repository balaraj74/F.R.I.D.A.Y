...
function resolveMode(cfg: FRIDAYConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveFRIDAYPackageRoot({
  ...
  let cfg: FRIDAYConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("friday configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("friday config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("friday setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("friday doctor --fix")}" to apply changes.`);
  }
