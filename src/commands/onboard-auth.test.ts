...
const requireAgentDir = () => {
  const agentDir = process.env.FRIDAY_AGENT_DIR;
  if (!agentDir) throw new Error("FRIDAY_AGENT_DIR not set");
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.FRIDAY_OAUTH_DIR;
  });

  it("writes auth-profiles.json under FRIDAY_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-oauth-"));
    process.env.FRIDAY_STATE_DIR = tempStateDir;
    process.env.FRIDAY_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FRIDAY_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FRIDAY_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-minimax-"));
    process.env.FRIDAY_STATE_DIR = tempStateDir;
    process.env.FRIDAY_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.FRIDAY_AGENT_DIR;
