...
beforeAll(() => {
  previousProfile = process.env.FRIDAY_PROFILE;
  process.env.FRIDAY_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.FRIDAY_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/friday-root.js", () => ({
  resolveFRIDAYPackageRoot: vi.fn().mockResolvedValue("/tmp/friday"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/friday/pnpm-lock.yaml",
      markerPath: "/tmp/friday/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("FRIDAY status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("friday status --all") ||
          l.includes("friday --profile isolated status --all") ||
          l.includes("friday status --all") ||
          l.includes("friday --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.FRIDAY_GATEWAY_TOKEN = "abcd1234";
    try {
