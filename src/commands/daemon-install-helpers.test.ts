...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/friday/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\friday\\src\\cli\\index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "/Users/me/friday/dist/cli/index.js"])).toBe(false);
  });
});
...
describe("buildGatewayInstallPlan", () => {
  it("uses provided nodePath and returns plan", async () => {
    ...
    mocks.buildServiceEnvironment.mockReturnValue({ FRIDAY_PORT: "3000" });
    ...
    expect(plan.environment).toEqual({ FRIDAY_PORT: "3000" });
  ...
  it("merges config env vars into the environment", async () => {
    ...
    expect(plan.environment.FRIDAY_PORT).toBe("3000");
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:friday|friday)( --profile isolated)? gateway install/,
    );
  });
});
