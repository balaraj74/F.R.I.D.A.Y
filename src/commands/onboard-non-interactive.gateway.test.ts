...
describe("onboard (non-interactive): gateway and remote auth", () => {
  const prev = {
    ...
    password: process.env.FRIDAY_GATEWAY_PASSWORD,
  };
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "friday-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.FRIDAY_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "friday");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.FRIDAY_STATE_DIR = stateDir;
    process.env.FRIDAY_CONFIG_PATH = path.join(stateDir, "friday.json");
