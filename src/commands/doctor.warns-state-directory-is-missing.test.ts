...
beforeEach(() => {
  ...
  resolveFRIDAYPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.FRIDAY_STATE_DIR;
  originalUpdateInProgress = process.env.FRIDAY_UPDATE_IN_PROGRESS;
  process.env.FRIDAY_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "friday-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalUpdateInProgress === undefined) {
    ...
    process.env.FRIDAY_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
...
const resolveFRIDAYPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadFRIDAYPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/friday-root.js", () => ({
  resolveFRIDAYPackageRoot,
}));
...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "friday-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.FRIDAY_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when FRIDAY_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/friday.json",
      exists: true,
      raw: "{}",
    ...
    process.env.FRIDAY_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
