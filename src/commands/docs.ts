...
const SEARCH_TOOL = "https://docs.friday.ai/mcp.SearchFRIDAY";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.friday.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('friday docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.friday.ai/");
      runtime.log(`Search: ${formatCliCommand('friday docs "your query"')}`);
    }
    return;
  }
