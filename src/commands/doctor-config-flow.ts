...
function stripUnknownConfigKeys(config: FRIDAYConfig): {
  config: FRIDAYConfig;
  removed: string[];
} {
  const parsed = FRIDAYSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as FRIDAYConfig;
...
function noteOpencodeProviderOverrides(cfg: FRIDAYConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.FRIDAY_CONFIG_PATH?.trim() || env.FRIDAY_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "friday.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "friday.json");
  ...
  let cfg: FRIDAYConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as FRIDAYConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("friday doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("friday doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "friday doctor --fix" to remove these keys.');
    }
  }
