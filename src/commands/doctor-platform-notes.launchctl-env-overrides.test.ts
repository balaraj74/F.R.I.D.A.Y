...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "FRIDAY_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("FRIDAY_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv FRIDAY_GATEWAY_TOKEN");
    expect(message).not.toContain("FRIDAY_GATEWAY_PASSWORD");
  });
