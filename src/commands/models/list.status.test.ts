...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveFRIDAYAgentDir: vi.fn().mockReturnValue("/tmp/friday-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/friday-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveFRIDAYAgentDir: mocks.resolveFRIDAYAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/friday-agent/auth-profiles.json");
