...
import { readConfigFileSnapshot, type FRIDAYConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: FRIDAYConfig, model: string): FRIDAYConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("friday plugins install")}\`.`,
    );
  }
