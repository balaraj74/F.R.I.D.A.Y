...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("friday models auth setup-token")}\` or \`${formatCliCommand("friday configure")}\`.`
          : `Run \`${formatCliCommand("friday configure")}\` or set an API key env var.`;
