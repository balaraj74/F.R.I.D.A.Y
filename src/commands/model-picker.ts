...
function resolveConfiguredModelRaw(cfg: FRIDAYConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: FRIDAYConfig): string[] {
...
export function applyPrimaryModel(cfg: FRIDAYConfig, model: string): FRIDAYConfig {
...
export function applyModelAllowlist(cfg: FRIDAYConfig, models: string[]): FRIDAYConfig {
  const defaults = cfg.agents?.defaults;
