...
import type { FRIDAYConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: FRIDAYConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("friday security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("friday configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("friday config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("friday doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("friday config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
