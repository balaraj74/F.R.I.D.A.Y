...
export function listAgentEntries(cfg: FRIDAYConfig): AgentEntry[] {
...
function resolveAgentName(cfg: FRIDAYConfig, agentId: string) {
...
function resolveAgentModel(cfg: FRIDAYConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: FRIDAYConfig): AgentSummary[] {
