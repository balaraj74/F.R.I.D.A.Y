...
import { resolveFRIDAYAgentDir } from "../agents/agent-paths.js";
...
import { ensureFRIDAYModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { FRIDAYConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.FRIDAY_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.FRIDAY_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.FRIDAY_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<FRIDAYConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.FRIDAY_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.FRIDAY_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveFRIDAYAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-live-state-"));
  ...
  process.env.FRIDAY_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.friday-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveFRIDAYAgentDir();
  const sanitizedCfg: FRIDAYConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "FRIDAY live tool probe (local, safe): " +
    ...
    process.env.FRIDAY_CONFIG_PATH = previous.configPath;
    process.env.FRIDAY_GATEWAY_TOKEN = previous.token;
    process.env.FRIDAY_SKIP_CHANNELS = previous.skipChannels;
    process.env.FRIDAY_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.FRIDAY_SKIP_CRON = previous.skipCron;
    process.env.FRIDAY_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.FRIDAY_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.FRIDAY_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureFRIDAYModelsJson(cfg);
      ...
      const rawModels = process.env.FRIDAY_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.friday-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
