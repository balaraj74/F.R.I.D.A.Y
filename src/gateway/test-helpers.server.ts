...
async function setupGatewayTestHome() {
  ...
  previousConfigPath = process.env.FRIDAY_CONFIG_PATH;
  previousSkipBrowserControl = process.env.FRIDAY_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.FRIDAY_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.FRIDAY_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "friday-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.FRIDAY_STATE_DIR = path.join(tempHome, ".friday");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "friday-test-"))
    : path.join(tempHome, ".friday-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) delete process.env.FRIDAY_CONFIG_PATH;
    else process.env.FRIDAY_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.FRIDAY_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.FRIDAY_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.FRIDAY_SKIP_GMAIL_WATCHER;
    else process.env.FRIDAY_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.FRIDAY_SKIP_CANVAS_HOST;
    else process.env.FRIDAY_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.FRIDAY_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.FRIDAY_GATEWAY_TOKEN = fallbackToken;
  }
