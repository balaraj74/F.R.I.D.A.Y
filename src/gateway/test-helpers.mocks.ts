...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `friday-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.FRIDAY_CONFIG_PATH = path.join(root, "friday.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "friday.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "friday-gateway-test"),
        ...fileDefaults,
