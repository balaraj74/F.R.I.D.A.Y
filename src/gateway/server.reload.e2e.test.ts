...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.FRIDAY_SKIP_CHANNELS;
    prevSkipGmail = process.env.FRIDAY_SKIP_GMAIL_WATCHER;
    process.env.FRIDAY_SKIP_CHANNELS = "0";
    delete process.env.FRIDAY_SKIP_GMAIL_WATCHER;
  });

  afterEach(() => {
    if (prevSkipChannels === undefined) {
      delete process.env.FRIDAY_SKIP_CHANNELS;
    } else {
      process.env.FRIDAY_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.FRIDAY_SKIP_GMAIL_WATCHER;
    } else {
      process.env.FRIDAY_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
