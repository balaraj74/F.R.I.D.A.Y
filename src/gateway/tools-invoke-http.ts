...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-friday-message-channel") ?? "");
  const accountId = getHeader(req, "x-friday-account-id")?.trim() || undefined;
  ...
  const allTools = createFRIDAYTools({
