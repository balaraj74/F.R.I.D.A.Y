...
describe("logs.tail", () => {
  ...
  it("falls back to latest rolling log file when today is missing", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-logs-"));
    const older = path.join(tempDir, "friday-2026-01-20.log");
    const newer = path.join(tempDir, "friday-2026-01-21.log");
    ...
    setLoggerOverride({ file: path.join(tempDir, "friday-2026-01-22.log") });
