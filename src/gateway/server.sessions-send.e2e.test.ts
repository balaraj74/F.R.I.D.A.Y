...
import { createFRIDAYTools } from "../agents/friday-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.FRIDAY_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.FRIDAY_GATEWAY_PORT = String(gatewayPort);
  process.env.FRIDAY_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.FRIDAY_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createFRIDAYTools().find((candidate) => candidate.name === "sessions_send");
