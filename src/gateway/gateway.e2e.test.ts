...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.FRIDAY_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "friday-gw-mock-home-"));
      ...
      process.env.FRIDAY_SKIP_BROWSER_CONTROL_SERVER = "1";

      const token = `test-${randomUUID()}`;
      process.env.FRIDAY_GATEWAY_TOKEN = token;

      const workspaceDir = path.join(tempHome, "friday");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.friday-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".friday");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "friday.json");
      ...
      process.env.FRIDAY_CONFIG_PATH = configPath;
      ...
      try {
        ...
        process.env.FRIDAY_CONFIG_PATH = prev.configPath;
        process.env.FRIDAY_GATEWAY_TOKEN = prev.token;
        process.env.FRIDAY_SKIP_CHANNELS = prev.skipChannels;
        process.env.FRIDAY_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.FRIDAY_SKIP_CRON = prev.skipCron;
        process.env.FRIDAY_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.FRIDAY_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "friday-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.FRIDAY_STATE_DIR = prev.stateDir;
      process.env.FRIDAY_CONFIG_PATH = prev.configPath;
      process.env.FRIDAY_GATEWAY_TOKEN = prev.token;
      process.env.FRIDAY_SKIP_CHANNELS = prev.skipChannels;
      process.env.FRIDAY_SKIP_GMAIL_WATCHER = prev.skipGmail;
