...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.FRIDAY_AGENT_DIR;
    else process.env.FRIDAY_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-chutes-"));
    process.env.FRIDAY_STATE_DIR = tempDir;
    process.env.FRIDAY_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
