...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "friday-models-" });
}

const MODELS_CONFIG: FRIDAYConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureFRIDAYModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureFRIDAYModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureFRIDAYModelsJson } = await import("./models-config.js");
      const { resolveFRIDAYAgentDir } = await import("./agent-paths.js");

      await ensureFRIDAYModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveFRIDAYAgentDir(), "models.json");
