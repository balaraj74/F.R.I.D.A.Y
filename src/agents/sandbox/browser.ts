...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    profiles: {
      friday: { cdpPort: params.cdpPort, color: DEFAULT_CLAWD_BROWSER_COLOR },
    },
  };
}
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "friday.sandboxBrowser": "1" },
    });
    ...
    args.push("-e", `FRIDAY_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push("-e", `FRIDAY_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
    args.push("-e", `FRIDAY_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `FRIDAY_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `FRIDAY_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, "friday")
    : null;
  const shouldReuse =
