...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "friday.sandbox=1");
  args.push("--label", `friday.sessionKey=${params.scopeKey}`);
  args.push("--label", `friday.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `friday.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "friday.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`friday sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`friday sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("friday sandbox recreate --all");
}
