...
import { createFRIDAYTools } from "./friday-tools.js";
import { __testing, createFRIDAYCodingTools } from "./pi-tools.js";
...
describe("createFRIDAYCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createFRIDAYCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "friday-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "friday-outside.txt");
