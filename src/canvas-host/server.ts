...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.fridayCanvasA2UIAction ||
        window.webkit.messageHandlers.fridayCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.fridayCanvasA2UIAction &&
        typeof window.fridayCanvasA2UIAction.postMessage === "function") ||
      (window.fridayCanvasA2UIAction &&
        typeof window.fridayCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.fridaySendUserAction === "function" ? window.fridaySendUserAction : undefined;
  if (!window.fridaySendUserAction && legacySend) {
    window.fridaySendUserAction = legacySend;
  }
  if (!window.fridaySendUserAction && typeof window.fridaySendUserAction === "function") {
    window.fridaySendUserAction = window.fridaySendUserAction;
  }
  const hasHelper = () =>
    typeof window.fridaySendUserAction === "function" ||
    typeof window.fridaySendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("friday:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android FRIDAY node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.fridaySendUserAction === "function"
        ? window.fridaySendUserAction
        : window.fridaySendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FRIDAY_SKIP_CANVAS_HOST)) return true;
...
export async function createCanvasHostHandler(
  ...
  const rootDir = resolveUserPath(opts.rootDir ?? path.join(os.homedir(), "friday", "canvas"));
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>FRIDAY Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
