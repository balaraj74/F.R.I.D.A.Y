...
import { ensureFRIDAYCliOnPath } from "../infra/path-env.js";
...
const execHostEnforced = process.env.FRIDAY_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.FRIDAY_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureFRIDAYCliOnPath({ pathEnv: process.env.PATH ?? "" });
