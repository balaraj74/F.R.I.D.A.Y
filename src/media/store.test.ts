...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "FRIDAY_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "friday-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.FRIDAY_STATE_DIR = path.join(home, ".friday");
    ...
    await fs.mkdir(path.join(home, ".friday"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.friday${path.sep}media`);
