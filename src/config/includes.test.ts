...
const ETC_FRIDAY_DIR = path.join(ROOT_DIR, "etc", "friday");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "friday.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcFRIDAYPath(...parts: string[]) {
  return path.join(ETC_FRIDAY_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcFRIDAYPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "friday.json"))).toEqual({
      shared: true,
    });
  });
});
