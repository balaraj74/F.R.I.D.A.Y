...
import type { FRIDAYConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: FRIDAYConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: FRIDAYConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = FRIDAYSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as FRIDAYConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as FRIDAYConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as FRIDAYConfig)),
    ),
  };
}
