...
import type { FRIDAYConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareFRIDAYVersions } from "./version.js";
...
function coerceConfig(value: unknown): FRIDAYConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as FRIDAYConfig;
}
...
function stampConfigVersion(cfg: FRIDAYConfig): FRIDAYConfig {
...
function warnIfConfigFromFuture(cfg: FRIDAYConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareFRIDAYVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer FRIDAY (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: FRIDAYConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): FRIDAYConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as FRIDAYConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as FRIDAYConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: FRIDAYConfig) {
    clearConfigCache();
...
// module scope. `FRIDAY_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FRIDAY_CONFIG_CACHE_MS?.trim();
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FRIDAY_DISABLE_CONFIG_CACHE?.trim()) return false;
...
export async function writeConfigFile(cfg: FRIDAYConfig): Promise<void> {
  clearConfigCache();
